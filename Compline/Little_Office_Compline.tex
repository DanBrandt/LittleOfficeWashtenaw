...
\usepackage{paracol}
\usepackage{mdframed}
\usepackage[paperheight=6in,paperwidth=4in,margin=0.33in,heightrounded]{geometry}

\usepackage[autocompile]{gregoriotex}

\titleformat{\chapter}[block]{\Large\scshape\filcenter}{}{1em}{}
...
% Font and score size settings:
%\grechangestyle{initial}{2s pt font}
%\grechangestaffsize{8}

\vspace*{1.25cm}

\begin{center}
	\textbf{\Large Compline of the Blessed Virgin Mary}\\
...
\begin{center}
\includegraphics[width=0.65\textwidth]{ChristGethsemani}
\end{center}
...
\latinenglishequal{
	Áperi, {\color{red}\maltese}\ Dómine, os meum ad benedicéndum nomen sanctum tuum:
...
\latinenglish{
	\gresetinitiallines{1}
	\gregorioscore{ps128_in_dir}
	
	\begin{verses}{1}
	
	\item Sæpe expugnavérunt me a juven\textit{tú}\textit{te} \textbf{me}a:~* étenim non potuérunt mi\textit{hi}.

	\item Supra dorsum meum fabricavérunt \textit{pec}\textit{ca}\textbf{tó}res:~* prolongavérunt iniquitátem su\textit{am}.
	
	\item Dóminus justus concídit cervíces \textit{pec}\textit{ca}\textbf{tó}rum:~* confundántur et convertántur retrórsum omnes, qui odérunt Si\textit{on}.
	
	\item Fiant sicut fœ\textit{num} \textit{tec}\textbf{tó}rum:~* quod priúsquam evellátur exáru\textit{it}:
	
	\item De quo non implévit manum su\textit{am} \textit{qui} \textbf{me}tit:~* et sinum suum qui manípulos cólli\textit{git}.
	
	\item Et non dixérunt qui præteríbant: Benedíctio Dó\textit{mi}\textit{ni} \textbf{su}per vos:~* benedíximus vobis in nómine Dómi\textit{ni}.
	
	\item \textit{\color{red}(bow)} Glória Pa\textit{tri}, \textit{et} \textbf{Fí}lio,~* et Spirítui Sanc\textit{to}.
	
	\item \textit{\color{red}(rise)} Sicut erat in princípio, et \textit{nunc}, \textit{et} \textbf{sem}per,~* et in s\'{\ae}cula sæculórum. A\textit{men}.
...
\latinenglish{
	\gresetinitiallines{1}
	\gregorioscore{ps129_in_dir}
	
	\begin{verses}{1}
	
	\item Fiant aures tuæ \textit{in}\textit{ten}\textbf{dén}tes:~* in vocem deprecatiónis me\textit{æ}.

	\item Si iniquitátes observá\textit{ve}\textit{ris}, \textbf{Dó}mine:~* Dómine, quis sustiné\textit{bit}?
	
	\item Quia apud te propiti\textit{á}\textit{ti}\textbf{o} est:~* et propter legem tuam sustínui te, Dómi\textit{ne}.
	
	\item Sustínuit ánima mea in \textit{ver}\textit{bo} \textbf{e}jus:~* sperávit ánima mea in Dómi\textit{no}.
	
	\item A custódia matutína us\textit{que} \textit{ad} \textbf{noc}tem:~* speret Israël in Dómi\textit{no}.
	
	\item Quia apud Dóminum mi\textit{se}\textit{ri}\textbf{cór}dia:~* et copiósa apud eum redémpti\textit{o}.
	
	\item Et ipse réd\textit{i}\textit{met} \textbf{Is}raël:~* ex ómnibus iniquitátibus e\textit{jus}.
...
\latinenglish{
	\gresetinitiallines{1}
	\gregorioscore{ps130_in_dir}
	
	\begin{verses}{1}
	
	\item Neque ambulá\textit{vi} \textit{in} \textbf{ma}gnis:~* neque in mirabílibus super \textit{me}.

	\item Si non humíliter \textit{sen}\textit{ti}\textbf{é}bam:~* sed exaltávi ánimam me\textit{am}.
	
	\item Sicut ablactátus est super \textit{ma}\textit{tre} \textbf{su}a:~* ita retribútio in ánima me\textit{a}.
	
	\item Speret Isra\textit{ël} \textit{in} \textbf{Dó}mino:~* ex hoc nunc et usque in sǽcu\textit{lum}.
	
	\item \textit{\color{red}(bow)} Glória Pa\textit{tri}, \textit{et} \textbf{Fí}lio,~* et Spirítui Sanc\textit{to}.
	
	\item \textit{\color{red}(rise)} Sicut erat in princípio, et \textit{nunc}, \textit{et} \textbf{sem}per,~* et in sǽcula sæculórum. A\textit{men}.
...
\heading{\subsection*{Throughout the Year and in Christmastide: Ecclus 24:24}}
%\latinenglish{
%	\gresetinitiallines{0}
...
\latinenglish{
	\gresetinitiallines{1}
	\gabcsnippet{(c3)E(h)go(h) ma(h)ter(h) pul(h)chræ(h) di(h)lec(h)ti(h)ó(h)nis,(f.) <c>+</c>(,) et(h) ti(h)mó(h)ris,(h) et(h) ag(h)ni(h)ti(h)ó(h)ni,(h'_) (,) et(h) sanc(h)tæ(h) spe(f)i.(e.f.) (::)}
	
	\gresetinitiallines{0}
	\gabcsnippet{(c3)<c><sp>R/</sp>.</c> De(h)o(h) grá(f)ti(e)as.(e.f.) (::)}
	
	\gresetinitiallines{0}
	\gabcsnippet{(c3)<c><sp>V/</sp>.</c> O(h)ra(h) pro(h) no(h)bis(h) sanc(h)ta(h) De(h)i(h) Gé(hr1)ni(f)trix.(f.) (::)}
	
	\gresetinitiallines{0}
	\gabcsnippet{(c3)<c><sp>R</sp>.</c> Ut(h) dig(h)ni(h) ef(h)fi(h)ci(h)á(h)mur(h) pro(h)mis(h)si(h)ó(h)ni(h)bus(h) Chri(hr1)sti.(f.) (::)}
...
%}
\latinenglish{
	Ecce Virgo concipiet, et pariet filium, et vocabitur nomen ejus Emmanuel. {\color{red}\GreDagger}\ Butyrem et \textit{mel co}\textbf{me}dat, ut sciat reprobare malu, * et eligere bonum.\\
