...
\rubric{\color{red} For `Christmastide', see page \underline{[]}.'}
...
\rubric{\color{red}The Officiant leads the following prayers:}
...
\latinenglish{
	\gresetinitiallines{0}
	\gabcsnippet{(c3) <c><sp>V/</sp>.</c> Fi(f)dé(f)li(f)um(f) á(f)ni(f)mæ(f) per(f) mi(f)se(f)ri(f)cór(f)di(f)am(f) De(f)i(f), re(f)qui(f)é(f)scant(f) in(f) pa(f)ce(f.). (::)}
	\gabcsnippet{(c3) <c><sp>R/</sp>.</c> Am(f.)en.(f.) (::)}
}{
