...
\latinenglish{
	\gresetinitiallines{0}
	\gabcsnippet{(c3)<sp>V/</sp>. Dif(h)fu(h)sa(h) est(h) gra(h)ti(h)a(h) in(h) la(h)bi(h)is(h) tu(h.)is(f.). (::)}
}{
	\Vbar. Grace was poured forth on thy lips.
}
\latinenglish{
	\gresetinitiallines{0}
	\gabcsnippet{(c3)<sp>R/</sp>. Prop(h)te(h)re(h)a(h) be(h)ne(h)di(h)xit(h) te(h) De(h)us(h) in(h) æ(h)ter(h.)num(f.). (::)}
}{
	\Rbar. Therefore hath the Lord blessed thee forever.
}
\end{latinenglishsection}
